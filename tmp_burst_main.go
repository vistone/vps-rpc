...
func main(){
	...
	cli, err := client.NewClient(&client.ClientConfig{Address: addr, Timeout: 20 * time.Second, InsecureSkipVerify: true, EnableConnectionPool: true, MaxPoolSize: 10})
	...
	for i:=1; i<=10; i++ {
		...
		if err != nil { fmt.Printf("[%d] err=%v latency=%v\n", i, err, elapsed); continue }
		if resp.Error != "" { fmt.Printf("[%d] respErr=%s latency=%v\n", i, resp.Error, elapsed); continue }
		fmt.Printf("[%d] status=%d bytes=%d latency=%v\n", i, resp.StatusCode, len(resp.Body), elapsed)
